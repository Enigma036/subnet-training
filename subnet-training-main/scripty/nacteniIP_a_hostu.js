...
function vypsani_hostu()
{
    ...
    if(pocet_hostu <= 5)
    {
        ...
        for (let i = 0; i < pocet_hostu; i++)
        {
            ...
            text += "<td><input class = \"hoste_rozsah_css\" type=\"number\" id=\"ro"+ i +"\" name=\"pocet_hostu\" min=\"1\" max=\"1000\" value=\""+ro[i]+"\" onkeyup=hlavni_kontrola(this) onchange=\"kontrola_prazdneho_pole(this)\"></td>";
            text += "<td><input class = \"hoste_rozsah_css\" type=\"number\" id=\"rd"+ i +"\" name=\"pocet_hostu\" min=\"1\" max=\"1000\" value=\""+rd[i]+"\" onkeyup=hlavni_kontrola(this) onchange=\"kontrola_prazdneho_pole(this)\"></td>";
            text += "</td><tr>";
        }
        text += "</table></div>";
    }
...
function nova_ip_a_novi_hoste()
{
    po_kliku_novi_hoste();
    nova_ip();
}

function uprava_pole_s_pocty_hostu(parametr)
{
    kontrola_prazdneho_pole(parametr, false);
    zmena_poctu_hostu();
}
